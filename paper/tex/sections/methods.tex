...
We assume that the Milky Way is composed of a thin disc, thick disc and a central bar/bulge. For each of these components we use a separate lookback time, radial and vertical distribution, which we combine into a single model, weighting each component by its stellar mass. \citet{Licquia+2015} gives that the stellar mass of the bulge is $0.91 \times 10^{10} \unit{M_{\odot}}$ and the stellar mass of the disc is $5.17 \times 10^{10} \unit{M_\odot}$, which we split equally between the thin and thick disc. \todo{need citation for equal mass discs}

\textit{Star formation history:} For the thin and thick discs we use the star formation history given by \citet[][Eq.~2]{Frankel+2018},
\begin{equation}\label{eq:thin_disc_tau}
    p(\tau) \propto \exp \qty(-\frac{(\tau_m - \tau)}{\tau_{\rm SFR}}),
\end{equation}
where $\tau$ is the lookback time (the amount of time elapsed between the binary's zero-age main sequence and today), $\tau_m = 12 \unit{Gyr}$ is the assumed age of the Milky Way and $\tau_{\rm SFR} = 6.8 \unit{Gyr}$ is the star formation timescale. Following \citet{Frankel+2018}, we assume that there is a transition from thick disc star formation to thin disc star formation $8 \unit{Gyr}$ ago and apply a sharp transition at this point. This has been shown to still reproduce the Milky Way structure well despite its simplicity \citep{Sharma+2020} \todo{I could write this bit better}. Therefore, for the thin disc we normalise this distribution on the range $[0, 8) \unit{Gyr}$ and for the thick disc we normalise on the range $[8, 12] \unit{Gyr}$.

For the bulge, we base our distribution on the one presented in \citet[][Fig.~7]{Bovy+2019}. We adapt this distribution to account for the possible bias of the sample, where fainter older stars are not observed \todo{I don't understand this well enough to write}. We therefore approximate the distribution using a $\beta(2,3)$ distribution, shifted and scaled such that stars are only formed in the range $[6, 12] \unit{Gyr}$.
...
\end{equation}
where $\alpha_{R_{\rm exp}} = 0.3$ is the inside-out growth parameter\footnote{In $R_{\rm exp}(\tau)$, we use 4 kpc instead of 3 kpc for the 0 Gyr exponential scale-length of the disc as it provides a better fit (correspondence with Neige Frankel)}. This scale length accounts for the inside-out growth of the thin disc and hence is age dependent. We assume $R_d = (1 / 0.43) \unit{kpc}$ for the thick disc \citep[][Table~1]{Bovy+2016} and $R_d = 1.5 \unit{kpc}$ for the bulge \citep{Bovy+2019}.
...
\textit{Metallicity-radius-time relation:} Most previous studies that predict a detection rate for LISA place binaries in the Milky Way independently of their age or evolution. We improve upon this as the first study to use an analytical model of the Milky Way that takes into account the galaxy's enrichment history. For this purpose, we apply the metallicity-radius-time relation from \citet{Frankel+2018}. The authors developed this model in order to measure the global efficiency of radial migration in the Milky Way and calibrated it using a sample of red clump stars measured with APOGEE \citep{Majewski+2017} and matched to stars in Gaia \citep{GaiaCollaboration+2016}.

The metallicity-radius-time relation is given by \citep[][Eq. 7]{Frankel+2018}

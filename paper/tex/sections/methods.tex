...
\subsubsection{Physical assumptions in our fiducial model}\label{sec:fiducial_physics}
...
For the bulge, we set $z_d = 1.5 \unit{kpc}$ such that the probability of forming a star approaches zero at approximately the co-rotation radius of the Galactic bar/bulge \citep{Bovy+2019}. \todo{@Neige, did I justify the bulge/bar height correctly? A: The vertical distribution should not influence at which radius the bar should stop forming stars -- more later}. We show the combination of these distributions in the last panel in the left half of Fig.~\ref{fig:galaxy_schematic}.

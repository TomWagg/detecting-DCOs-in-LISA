...
In Figure~\ref{fig:ecc_low_kick_variation}, we investigate how decreasing the magnitude of natal kicks as a result of core-collapse supernovae affects the eccentricity distribution of detectable DCOs. For reference, we show the fiducial distributions from the lower left panel of Fig.~\ref{fig:fiducial_pdf_distributions} as dashed lines. In the main curves, we reduce the velocity dispersion for core-collapse supernovae from $265 \unit{km}{s^{-1}}$ to $30 \unit{km}{s^{-1}}$ (using model \modSigLower{} instead of \modFid{}).



\subsubsection{Other interesting features}
\todo{explore whether there is anything interesting the CE ones}
